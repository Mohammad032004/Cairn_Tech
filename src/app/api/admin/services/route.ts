...
export async function GET(req: NextRequest) {
  try {
    ...
    await dbConnect();
    return NextResponse.json({ success: true, data: await Service.find().sort({ order: 1 }) });
...
export async function POST(req: NextRequest) {
  try {
    if (!requireAdmin(req)) return NextResponse.json({ success: false }, { status: 401 });
    await dbConnect();
    const doc = await Service.create(await req.json());
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const doc = await Service.findByIdAndUpdate(id, rest, { new: true });
    return NextResponse.json({ success: true, data: doc });
  } catch (err: any) { return NextResponse.json({ success: false, message: err.message }, { status: 500 }); }
}
